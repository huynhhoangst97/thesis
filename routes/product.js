...
router.post("/", (req, res) => {
    ...
    anchor.find({ tag: "0296" }, (err, result) => {
        anchor_2.xcale = result[0]["location"]["xcale"];
        anchor_2.ycale = result[0]["location"]["ycale"];
    });
    anchor.find({ tag: "0384" }, (err, result) => {
        anchor_3.xcale = result[0]["location"]["xcale"];
        anchor_3.ycale = result[0]["location"]["ycale"];
    });
    anchor.find({ tag: "4134" }, (err, result) => {
        anchor_4.xcale = result[0]["location"]["xcale"];
        anchor_4.ycale = result[0]["location"]["ycale"];
    });
    ...
    res.send({ tag1_xcale: toaDo_1.xcale, tag1_ycale: toaDo_1.ycale, tag2_xcale: toaDo_2.xcale, tag2_ycale: toaDo_2.ycale, anchor1_xcale: anchor_1.xcale, anchor1_ycale: anchor_1.ycale, anchor2_xcale: anchor_2.xcale, anchor2_ycale: anchor_2.ycale, anchor3_xcale: anchor_3.xcale, anchor3_ycale: anchor_3.ycale, anchor4_xcale: anchor_4.xcale, anchor4_ycale: anchor_4.ycale , gateway1_tag: gateway_1.tag,gateway1_id : gateway_1.id })
    res.send("thanh cong");
});
